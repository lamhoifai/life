<div class="page-header">
	<div class="btn-group pull-right">
		<?php echo $this->Html->link($this->BootstrapIcon->css('pencil').' '.__('Edit Role'), array('action' => 'edit', $role['Role']['id']), array('escape'=>false,'class'=>'btn btn-default btn-small')); ?>
        <?php if (!count($role['User'])) { ?>
            <?php echo $this->Form->postLink($this->BootstrapIcon->css('trash','white').' '.__('Delete Role'), array('action' => 'delete', $role['Role']['id']), array('escape'=>false,'class'=>'btn btn-danger btn-small'), __('Are you sure you want to delete # %s?', $role['Role']['id'])); ?>
        <?php } ?>
		<?php echo $this->Html->link($this->BootstrapIcon->css('list-alt').' '.__('List Roles'), array('action' => 'index'), array('escape'=>false,'class'=>'btn btn-default btn-small')); ?>
		<?php echo $this->Html->link($this->BootstrapIcon->css('plus','white').' '.__('New Role'), array('action' => 'add'), array('escape'=>false,'class'=>'btn btn-primary btn-small')); ?>
	</div>
...
<div class="row">
	<div class="span12">
		<dl>
            ...
            <dd><span class="label label-info"><?php echo h($role['Role']['name']);?></span></dd>
			...
			<tr>
				...
				<td>
					<div class="btn-group">
                        ...
                        <?php } ?>
						<?php //echo $this->Form->postLink($this->BootstrapIcon->css('trash','white').' '.__('Delete'), array('controller' => 'users', 'action' => 'delete', $user['id']), array('escape'=>false, 'class'=>'btn btn-small btn-danger'), __('Are you sure you want to delete # %s?', $role['Role']['id'])); ?>
					</div>
				</td>
			</tr>
		<?php endforeach; ?>
